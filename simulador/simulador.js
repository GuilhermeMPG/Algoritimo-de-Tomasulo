Estado = class {
     ...
     alocaFuMem(uf, instrucao, estadoInstrucao) {
          ...
          if (instrucao.operacao === 'FS') {
               ...
               let UFQueTemQueEsperar =
                    this.estacaoRegistradores[instrucao.registradorR];

               // caso o nome seja de uma das unidades funcionais, marca que tem que esperar ela
               if (
                    UFQueTemQueEsperar in this.unidadesFuncionais ||
                    UFQueTemQueEsperar in this.unidadesFuncionaisMemoria
               )
          ...
          let UFintQueTemQueEsperar =
               this.estacaoRegistradores[instrucao.registradorT];

          // caso o nome seja de uma das unidades funcionais, marca que tem que esperar ela
          if (
               UFintQueTemQueEsperar in this.unidadesFuncionais ||
               UFintQueTemQueEsperar in this.unidadesFuncionaisMemoria
          )
     ...
     issueNovaInstrucao() {
          ...
          if (novaInstrucao) {
               let ufInstrucao = this.verificaUFInstrucao(
                    novaInstrucao.instrucao
               ); // verifica qual unidade essa instrucao usa
               ...
               if (UFParaUsar) {
                    ...
                    if (
                         ...
                         this.escreveEstacaoRegistrador(
                              novaInstrucao.instrucao,
                              UFParaUsar.nome
                         );
               }
...
function getConfig() {
     ...
     if (
          ...
          alert(
               'A quantidade de ciclos por instrução, para todas as unidades, deve ser de no mínimo 1 ciclo!'
          );
          return null;
     }
     ...
     if (unidades['Load'] < 1 || unidadesMem['Store'] < 1) {
          alert(
               'A quantidade de unidades funcionais de memória deve ser no mínimo 1!'
          );
          return;
     }
...
function registradorInvalidoR(registrador) {
     return (
          registrador[0] != 'R' ||
          registrador.replace('R', '') == '' ||
          isNaN(registrador.replace('R', '')) ||
...
function registradorInvalidoF(registrador) {
     return (
          registrador[0] != 'F' ||
          registrador.replace('F', '') == '' ||
          registrador.replace('F', '') % 2 != 0 ||
          registrador.replace('F', '') > 30 ||
...
function validaInstrucao(instrucao) {
     ...
     if (unidade == 'Load' || unidade == 'Store') {
          ...
          if (comando == 'FL' || comando == 'FS') {
               if (
                    registradorInvalidoF(instrucao['r']) ||
                    isNaN(parseInt(instrucao['s'])) ||
                    registradorInvalidoR(instrucao['t'])
               ) {
     ...
     if (
          registradorInvalidoF(instrucao['r']) ||
          registradorInvalidoF(instrucao['s']) ||
          registradorInvalidoF(instrucao['t'])
     ) {
          alertValidaInstrucao(instrucao);
          return false;
     }
...
function atualizaTabelaBufferReordenamentoHTML(tabelaInsts) {
     for (let i in tabelaInsts) {
          const inst = tabelaInsts[i];
          console.log(
               'EXECUÇÃO COMPLETA INSTRUCAO TESTE',
               inst['busy'],
               inst['posicao'],
               inst['instrucao'].operacao
          );
          ...
          if (inst['exeCompleta'] != null) {
               ...
               if (
                    inst['instrucao'].operacao == 'FS' ||
                    inst['instrucao'].operacao == 'FL'
               ) {
                    $(`#${inst['posicao']}_value`).text(
                         'Mem[' +
                              inst['instrucao'].registradorS +
                              ' + Regs[' +
                              inst['instrucao'].registradorT +
                              ']]'
                    );
               } else if (
               ...
               } else if (inst['instrucao'].operacao == 'FSUB') {
                    $(`#${inst['posicao']}_value`).text(
                         inst['instrucao'].registradorS +
                              ' - ' +
                              inst['instrucao'].registradorT
                    );
               } else if (inst['instrucao'].operacao == 'FMUL') {
                    $(`#${inst['posicao']}_value`).text(
                         inst['instrucao'].registradorS +
                              ' * ' +
                              inst['instrucao'].registradorT
                    );
               } else if (inst['instrucao'].operacao == 'FDIV') {
                    $(`#${inst['posicao']}_value`).text(
                         inst['instrucao'].registradorS +
                              ' / ' +
                              inst['instrucao'].registradorT
          ...
          //      inst['exeCompleta'] ? inst['exeCompleta'] : ''
          // );
...
function atualizaTabelaEstadoUFHTML(ufs) {
     for (let i in ufs) {
          const uf = ufs[i];
          $(`#${uf['nome']}_tempo`).text(
               uf['tempo'] !== null ? uf['tempo'] : ''
          );
          $(`#${uf['nome']}_ocupado`).text(uf['ocupado'] ? 'sim' : 'não');
          $(`#${uf['nome']}_operacao`).text(
               uf['operacao'] ? uf['operacao'] : ''
          );
...
function gerarTabelaBufferReordenamento(diagrama) {
     ...
     for (let i = 0; i < diagrama.configuracao['numInstrucoes']; ++i) {
          ...
          s +=
           ...
           <td id="` +
               i +
               `_busy">não</td> <td>${instrucao['operacao']} ${instrucao['registradorR']}, ${instrucao['registradorS']}, ${instrucao['registradorT']}</td> <td id="` +
               i +
               `_estado">Issue</td>
           <td id="` +
               i +
...
function gerarTabelaEstadoUFHTML(diagrama) {
     ...
     for (let key in diagrama.unidadesFuncionaisMemoria) {
          ...
          s += `<tr>
             <td>${ufMem['nome']}</td> <td id="${ufMem['nome']}_ocupado">não</td>
...
function atualizaTabelaEstadoUFMemHTML(ufsMem) {
     ...
     for (let key in ufsMem) {
          ...
          $(`#${ufMem['nome']}_operacao`).text(
               ufMem['operacao'] ? ufMem['operacao'] : ''
          );
...
function geraTabelaParaInserirInstrucoes(nInst) {
     var tabela =
          "<table id='tabelaInst' class='table table-striped table-hover'>";
     for (var i = 0; i < nInst; i++) {
          var d = 'D' + i;
          var r = 'R' + i;
          var s = 'S' + i;
          ...
          tabela +=
               '<tr>' +
               '<td>' +
               '<select class=\'form-control\' size="1" name="' +
               d +
               '" id="' +
               d +
               '">' +
